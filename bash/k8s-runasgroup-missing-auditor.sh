...
# k8s-runasgroup-missing-auditor.sh
# Report pods where pod-level runAsGroup is not set in securityContext.
...
Notes:
  - Flags active pods that do not define pod securityContext.runAsGroup.

Examples:
  bash/k8s-runasgroup-missing-auditor.sh
  bash/k8s-runasgroup-missing-auditor.sh --namespace production
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, runasgroup_missing_findings:$findings}'
else
  echo "K8s RunAsGroup Missing Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No pods missing pod-level runAsGroup were found."
  else
    echo "Pods missing pod-level runAsGroup: $finding_count"
    jq -r '.[] | "- \(.namespace)/\(.pod) owner=\(.owner_kind)/\(.owner_name)"' <<< "$findings_json"
  fi
fi
