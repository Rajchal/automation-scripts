...
# k8s-pod-serviceaccount-default-auditor.sh
# Report pods using the default ServiceAccount (or none specified).
...
Notes:
  - Flags pods where spec.serviceAccountName is omitted or set to "default".
  - Using the default ServiceAccount may grant broad permissions; prefer dedicated service accounts.

Examples:
  bash/k8s-pod-serviceaccount-default-auditor.sh
  bash/k8s-pod-serviceaccount-default-auditor.sh --namespace production
  bash/k8s-pod-serviceaccount-default-auditor.sh --output json --no-fail
EOF
}
...
findings_json="$(jq -c '
  [
    ...
    | {
        ...
        issue: "UsingDefaultServiceAccount"
      }
  ]
' <<< "$pods_json")"
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, default_serviceaccount_findings:$findings}'
else
  echo "K8s Pod Default ServiceAccount Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No pods using the default ServiceAccount were found."
  else
    echo "Pods using the default ServiceAccount: $finding_count"
    jq -r '.[] | "- \(.namespace)/\(.pod) serviceAccount=\(.service_account)"' <<< "$findings_json"
  fi
fi
