...
# k8s-hostaliases-usage-auditor.sh
# Report active pods that define hostAliases.
...
Notes:
  - Flags active pods where spec.hostAliases is non-empty.

Examples:
  bash/k8s-hostaliases-usage-auditor.sh
  bash/k8s-hostaliases-usage-auditor.sh --namespace production
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    --argjson pods_with_hostaliases "$findings_json" \
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, pods_with_hostaliases:$pods_with_hostaliases}'
else
  echo "K8s HostAliases Usage Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No active pods with hostAliases were found."
  else
    echo "Active pods with hostAliases: $finding_count"
    jq -r '.[] | "- \(.namespace)/\(.pod) owner=\(.owner_kind)/\(.owner_name) aliases=\(.host_aliases | map(.ip + ":" + (.hostnames | join("|"))) | join(","))"' <<< "$findings_json"
  fi
fi
