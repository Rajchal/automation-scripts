...
# k8s-hostpid-usage-auditor.sh
# Report active pods running with hostPID=true.
...
Notes:
  - hostPID allows processes in the pod to see and interact with the node PID namespace.

Examples:
  bash/k8s-hostpid-usage-auditor.sh
  bash/k8s-hostpid-usage-auditor.sh --namespace kube-system
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    --argjson pods_using_hostpid "$findings_json" \
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, pods_using_hostpid:$pods_using_hostpid}'
else
  echo "K8s HostPID Usage Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No active pods using hostPID were found."
  else
    echo "Active pods using hostPID: $finding_count"
