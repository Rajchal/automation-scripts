...
# k8s-procmount-unmasked-auditor.sh
# Report containers with securityContext.procMount set to Unmasked.
...
Notes:
  - Flags containers and initContainers with procMount=Unmasked.

Examples:
  bash/k8s-procmount-unmasked-auditor.sh
  bash/k8s-procmount-unmasked-auditor.sh --namespace production
...
findings_json="$(jq -c '
  [
    ...
    | [
        ((.spec.containers // [])[]? | {container_type:"container", name:(.name // "unknown"), proc_mount:(.securityContext.procMount // "Default")} ),
        ((.spec.initContainers // [])[]? | {container_type:"initContainer", name:(.name // "unknown"), proc_mount:(.securityContext.procMount // "Default")} )
      ]
    | .[]
    | select(.proc_mount == "Unmasked")
    | {
        namespace: $ns,
        pod: $pod,
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, procmount_unmasked_findings:$findings}'
else
  echo "K8s ProcMount Unmasked Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No containers with procMount=Unmasked were found."
  else
    echo "Containers with procMount=Unmasked: $finding_count"
    jq -r '.[] | "- \(.namespace)/\(.pod) type=\(.container_type) container=\(.container_name) procMount=\(.proc_mount)"' <<< "$findings_json"
  fi
fi
