...
# k8s-fsgroup-missing-auditor.sh
# Report active pods where spec.securityContext.fsGroup is not set.
...
Notes:
  - Flags active pods where pod-level securityContext.fsGroup is unset.

Examples:
  bash/k8s-fsgroup-missing-auditor.sh
  bash/k8s-fsgroup-missing-auditor.sh --namespace production
...
findings_json="$(jq -c '
  [
    ...
    | select(.spec.securityContext.fsGroup == null)
    | {
        namespace: .metadata.namespace,
        pod: .metadata.name,
        owner_kind: (.metadata.ownerReferences[0].kind // "Pod"),
        owner_name: (.metadata.ownerReferences[0].name // .metadata.name),
        run_as_user: (.spec.securityContext.runAsUser // "unset"),
        run_as_group: (.spec.securityContext.runAsGroup // "unset")
      }
  ]
' <<< "$pods_json")"
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    --argjson pods_missing_fsgroup "$findings_json" \
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, pods_missing_fsgroup:$pods_missing_fsgroup}'
else
  echo "K8s fsGroup Missing Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No active pods with missing fsGroup were found."
  else
    echo "Active pods missing fsGroup: $finding_count"
    jq -r '.[] | "- \(.namespace)/\(.pod) owner=\(.owner_kind)/\(.owner_name) runAsUser=\(.run_as_user) runAsGroup=\(.run_as_group)"' <<< "$findings_json"
  fi
fi
