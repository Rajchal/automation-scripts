...
# k8s-pod-hostpath-auditor.sh
# Report containers that mount hostPath volumes.
...
Notes:
  - Flags containers (regular and init) that mount hostPath volumes.
  - hostPath mounts can expose node filesystems to pods and should be reviewed carefully.

Examples:
  bash/k8s-pod-hostpath-auditor.sh
  bash/k8s-pod-hostpath-auditor.sh --namespace kube-system
  bash/k8s-pod-hostpath-auditor.sh --output json --no-fail
EOF
}
  ...
  [
    ...
    | (
        ($pod.spec.volumes[]? // []
          ...
          | {namespace: $pod.metadata.namespace, pod: $pod.metadata.name, volume: .name, hostPath: .hostPath.path, issue: "HostPathVolumeUsed"}
        )
      )
  ]
' <<< "$pods_json")"
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, hostpath_findings:$findings}'
else
  echo "K8s Pod HostPath Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No hostPath volume mounts were found."
  else
    echo "HostPath volume mounts: $finding_count"
    jq -r '.[] | "- \(.namespace)/\(.pod) volume=\(.volume) hostPath=\(.hostPath)"' <<< "$findings_json"
  fi
fi
