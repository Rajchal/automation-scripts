...
# k8s-hostpath-usage-auditor.sh
# Report pods using hostPath volumes, which can be high-risk in multi-tenant clusters.
...
Examples:
  bash/k8s-hostpath-usage-auditor.sh
  bash/k8s-hostpath-usage-auditor.sh --namespace kube-system
  bash/k8s-hostpath-usage-auditor.sh --output json --no-fail
EOF
}
...
findings_json="$(jq -c '
  [
    ...
    | .spec.volumes[]? as $v
    | select($v.hostPath != null)
    | {
        namespace: $ns,
        pod: $pod,
        volume_name: ($v.name // "unknown"),
        host_path: ($v.hostPath.path // ""),
        host_path_type: ($v.hostPath.type // "")
      }
  ]
' <<< "$pods_json")"
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, hostpath_usage:$findings}'
else
  echo "K8s HostPath Usage Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No hostPath usage detected in checked pods."
  else
    echo "Pods using hostPath volumes: $finding_count"
    jq -r '.[] | "- \(.namespace)/\(.pod) volume=\(.volume_name) path=\(.host_path) type=\(.host_path_type)"' <<< "$findings_json"
  fi
fi
