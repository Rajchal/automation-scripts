...
# k8s-pod-hostnetwork-auditor.sh
# Report pods that use hostNetwork: true
...
Notes:
  - Flags pods where spec.hostNetwork is true.
  - Running pods with hostNetwork can expose host networking and bypass pod network isolation.

Examples:
  bash/k8s-pod-hostnetwork-auditor.sh
  bash/k8s-pod-hostnetwork-auditor.sh --namespace kube-system
  bash/k8s-pod-hostnetwork-auditor.sh --output json --no-fail
EOF
}
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, hostNetwork_findings:$findings}'
else
  echo "K8s Pod HostNetwork Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No pods with hostNetwork: true were found."
  else
    echo "Pods with hostNetwork: true: $finding_count"
    jq -r '.[] | "- \(.namespace)/\(.pod) hostNetwork=\(.hostNetwork)"' <<< "$findings_json"
  fi
fi
