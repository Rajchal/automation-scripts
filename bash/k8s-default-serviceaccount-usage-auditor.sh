...
# k8s-default-serviceaccount-usage-auditor.sh
# Report active pods running with the default ServiceAccount.
...
Notes:
  - A pod is flagged when spec.serviceAccountName is unset or set to "default".

Examples:
  bash/k8s-default-serviceaccount-usage-auditor.sh
  bash/k8s-default-serviceaccount-usage-auditor.sh --namespace production
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    --argjson pods_using_default_serviceaccount "$findings_json" \
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, pods_using_default_serviceaccount:$pods_using_default_serviceaccount}'
else
  echo "K8s Default ServiceAccount Usage Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No active pods using the default ServiceAccount were found."
  else
    echo "Active pods using default ServiceAccount: $finding_count"
    jq -r '.[] | "- \(.namespace)/\(.pod) owner=\(.owner_kind)/\(.owner_name) sa=\(.service_account)"' <<< "$findings_json"
  fi
fi
