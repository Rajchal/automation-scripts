...
# k8s-runasuser-auditor.sh
# Report pods with containers that do not set securityContext.runAsUser.
...
Notes:
  - Flags containers and initContainers where runAsUser is unset.

Examples:
  bash/k8s-runasuser-auditor.sh
  bash/k8s-runasuser-auditor.sh --namespace production
...
findings_json="$(jq -c '
  [
    ...
    | [
        ((.spec.containers // [])[]? | {container_type:"container", container_name:(.name // "unknown"), runAsUser:(.securityContext.runAsUser // null)}),
        ((.spec.initContainers // [])[]? | {container_type:"initContainer", container_name:(.name // "unknown"), runAsUser:(.securityContext.runAsUser // null)})
      ]
    | .[]
    | select(.runAsUser == null)
    | {
        namespace: $ns,
        pod: $pod,
        container_type: .container_type,
        container_name: .container_name,
        issue: "RunAsUserUnset"
      }
  ]
' <<< "$pods_json")"
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, runasuser_unset_findings:$findings}'
else
  echo "K8s RunAsUser Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "All checked containers explicitly set runAsUser."
  else
    echo "Containers with unset runAsUser: $finding_count"
