...
# k8s-supplemental-groups-usage-auditor.sh
# Report active pods that set spec.securityContext.supplementalGroups.
...
Notes:
  - Flags active pods with one or more supplementalGroups configured.

Examples:
  bash/k8s-supplemental-groups-usage-auditor.sh
  bash/k8s-supplemental-groups-usage-auditor.sh --namespace production
...
findings_json="$(jq -c '
  [
    ...
    | select(((.spec.securityContext.supplementalGroups // []) | length) > 0)
    | {
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    --argjson pods_with_supplemental_groups "$findings_json" \
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, pods_with_supplemental_groups:$pods_with_supplemental_groups}'
else
  echo "K8s Supplemental Groups Usage Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No active pods with supplementalGroups were found."
  else
    echo "Active pods with supplementalGroups: $finding_count"
    jq -r '.[] | "- \(.namespace)/\(.pod) owner=\(.owner_kind)/\(.owner_name) supplementalGroups=\(.supplemental_groups | map(tostring) | join(","))"' <<< "$findings_json"
  fi
fi
