...
# k8s-pod-hostpid-hostipc-auditor.sh
# Report pods that enable hostPID or hostIPC
...
Notes:
  - Flags pods where spec.hostPID or spec.hostIPC is true.
  - Enabling host namespaces breaks pod isolation and should be reviewed.

Examples:
  bash/k8s-pod-hostpid-hostipc-auditor.sh
  bash/k8s-pod-hostpid-hostipc-auditor.sh --namespace kube-system
  bash/k8s-pod-hostpid-hostipc-auditor.sh --output json --no-fail
EOF
}
...
findings_json="$(jq -c '
  [
    ...
    | select((.spec.hostPID // false) == true or (.spec.hostIPC // false) == true)
    | {
        namespace: .metadata.namespace,
        pod: .metadata.name,
        hostPID: (.spec.hostPID // false),
        hostIPC: (.spec.hostIPC // false),
        issue: "HostNamespaceEnabled"
      }
  ]
' <<< "$pods_json")"
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, host_namespace_findings:$findings}'
else
  echo "K8s Pod HostPID/HostIPC Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No pods with hostPID or hostIPC enabled were found."
  else
    echo "Pods with hostPID/hostIPC enabled: $finding_count"
    jq -r '.[] | "- \(.namespace)/\(.pod) hostPID=\(.hostPID) hostIPC=\(.hostIPC)"' <<< "$findings_json"
  fi
fi
