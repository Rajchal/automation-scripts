...
# k8s-hostipc-usage-auditor.sh
# Report active pods running with hostIPC=true.
...
Notes:
  - hostIPC allows pods to share the node IPC namespace.

Examples:
  bash/k8s-hostipc-usage-auditor.sh
  bash/k8s-hostipc-usage-auditor.sh --namespace kube-system
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    --argjson pods_using_hostipc "$findings_json" \
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, pods_using_hostipc:$pods_using_hostipc}'
else
  echo "K8s HostIPC Usage Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No active pods using hostIPC were found."
  else
    echo "Active pods using hostIPC: $finding_count"
