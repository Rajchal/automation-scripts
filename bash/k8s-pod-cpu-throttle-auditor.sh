...
# k8s-pod-cpu-throttle-auditor.sh
# Report pods with containers or initContainers that set CPU limit without CPU request.
...
Notes:
  - Flags containers and initContainers that set a CPU limit but do not set a CPU request.
  - This helps identify workloads that may be subject to CPU throttling.

Examples:
  bash/k8s-pod-cpu-throttle-auditor.sh
  bash/k8s-pod-cpu-throttle-auditor.sh --namespace production
  bash/k8s-pod-cpu-throttle-auditor.sh --output json --no-fail
EOF
}
...
findings_json="$(jq -c '
  [
    ...
    | .[]
    | select(.cpu_limit != null and .cpu_request == null)
    | {
        namespace: $ns,
        pod: $pod,
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, cpu_throttle_findings:$findings}'
else
  echo "K8s Pod CPU Throttle Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No containers with CPU limit and missing CPU request were found."
  else
    echo "Containers with CPU limit but no request: $finding_count"
    jq -r '.[] | "- \(.namespace)/\(.pod) type=\(.container_type) container=\(.container_name) limit=\(.cpu_limit)"' <<< "$findings_json"
  fi
fi
