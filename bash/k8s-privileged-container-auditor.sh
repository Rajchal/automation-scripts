...
# k8s-privileged-container-auditor.sh
# Report pods running privileged containers or init containers.
...
Examples:
  bash/k8s-privileged-container-auditor.sh
  bash/k8s-privileged-container-auditor.sh --namespace production
  bash/k8s-privileged-container-auditor.sh --output json --no-fail
EOF
}
...
findings_json="$(jq -c '
  [
    ...
    | [
        ((.spec.containers // [])[]? | {container_type:"container", name:(.name // "unknown"), privileged:(.securityContext.privileged // false)}),
        ((.spec.initContainers // [])[]? | {container_type:"initContainer", name:(.name // "unknown"), privileged:(.securityContext.privileged // false)})
      ]
    | .[]
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, privileged_container_findings:$findings}'
else
  echo "K8s Privileged Container Auditor"
