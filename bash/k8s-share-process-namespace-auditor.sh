...
# k8s-share-process-namespace-auditor.sh
# Report active pods running with shareProcessNamespace=true.
...
Notes:
  - shareProcessNamespace=true allows containers in the same pod to see each other's processes.

Examples:
  bash/k8s-share-process-namespace-auditor.sh
  bash/k8s-share-process-namespace-auditor.sh --namespace kube-system
...
findings_json="$(jq -c '
  [
    ...
    | select((.spec.shareProcessNamespace // false) == true)
    | {
...
if [[ "$OUTPUT" == "json" ]]; then
  jq -n \
    ...
    --argjson pods_sharing_process_namespace "$findings_json" \
    '{scope:$scope, context:$context, selector:$selector, pod_count:$pod_count, pods_sharing_process_namespace:$pods_sharing_process_namespace}'
else
  echo "K8s Share Process Namespace Auditor"
  ...
  if [[ "$finding_count" -eq 0 ]]; then
    echo "No active pods using shareProcessNamespace were found."
  else
    echo "Active pods using shareProcessNamespace: $finding_count"
    jq -r '.[] | "- \(.namespace)/\(.pod) owner=\(.owner_kind)/\(.owner_name) node=\(.node)"' <<< "$findings_json"
  fi
fi
